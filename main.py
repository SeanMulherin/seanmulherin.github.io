...
def house_fcst(name, period):
    ...
    sns.despine()
    plt.title(f"AVG Monthly House Price - {name}", size='x-large', color='blue')
    plt.xlabel("Date")
    plt.ylabel("AVG Selling Price (USD)")
    plt.plot(df.index[-1], df.y[-1], 'bo')
    plt.text(df.index[-1], df.y[-1] + 10000, f'${obs_price:.2f}')
    ...
    components_fig = model.plot_components(pred, figsize=(12, 7))
